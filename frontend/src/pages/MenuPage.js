...
function MenuPage() {
  ...
  // fetch menu, station and business hours
  useEffect(() => {
    fetch(`${API}/menu`)
      .then(res => res.json())
      .then(setMenu)
  ...
  const fetchTrains = () => {
    ...
    fetch(`${API}/trains/arrivals?stationName=Cramlington`)
      .then(res => res.json())
      .then(data => {
        const trainList = Array.isArray(data)
          ? data
          : [];
        ...
        // keep selected train updated
        if (trainList.length > 0) {
          setSelectedTrain(prev => {
            if (!prev) return trainList[0];

            const refreshed = trainList.find(
              t => t.trainId === prev.trainId
            );
  ...
  // refresh trains every 60 seconds
  useEffect(() => {
    fetchTrains();
  ...
  // convert time into full date and time
  const toDatedTime = (time) => {
    ...
    return `${date} ${String(hour).padStart(
      2,
      "0"
    )}:${String(minute || 0).padStart(
      2,
      "0"
    )}`;
  };

  // create pickup time slots every 5 minutes
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const timeSlots = useMemo(() => {
    ...
    while (
      ...
      // only show future times if shop is open now
      if (
        todayHours &&
        !todayHours.closed &&
        isOpenNow
  ...
  const imageMap = {
    "Americano":
      "https://images.unsplash.com/photo-1580661869408-55ab23f2ca6e?w=500",

    "Americano with milk":
      "https://images.unsplash.com/photo-1565434007235-d3a18c2e0954?w=500",

    "Latte":
      "https://images.unsplash.com/photo-1593443320739-77f74939d0da?w=500",

    "Cappuccino":
      "https://images.unsplash.com/photo-1512568400610-62da28bc8a13?w=500",

    "Hot Chocolate":
      "https://images.unsplash.com/photo-1608651057580-4a50b2fc2281?w=500",

    "Mocha":
      "https://images.unsplash.com/photo-1618576230663-9714aecfb99a?w=500",

    "Mineral Water":
      "https://plus.unsplash.com/premium_photo-1681236320994-3395e842ed81?w=500"
  };
  ...
  const placeOrder = async () => {
    ...
    if (
      ...
      alert(
        "This train is cancelled. Please select another."
      );

      return;
    }

    // check train time with business hours
    if (pickupType === "train") {
      const arrTime =
  ...
  const preOrderLabel = () => {
    if (
      ...
      if (next) {
        return `Pre-order for ${next.hours.dayOfWeek} ${next.hours.openTime}–${next.hours.closeTime}`;
      }
    }
    ...
    <div className="bg-[#f5f1eb] min-h-screen p-4 sm:p-6 pb-24 font-sans">
      ...
      <div className="flex flex-col lg:flex-row gap-6 max-w-[1200px] mx-auto relative">
        ...
        <div className="flex-1 grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4 content-start">
          ...
          {menu.map(item => {
            ...
            return (
              <div
                key={item.id}
                className="bg-white rounded-2xl overflow-hidden shadow-[0_2px_10px_rgba(0,0,0,0.07)] flex flex-col"
              >
                <img
                  src={
                    imageMap[item.name] ||
                ...
                <div className="p-3 flex flex-col flex-1">
                  ...
                  <div className="font-semibold mb-2 text-[13px] sm:text-[14px] leading-tight flex-1">
                    {item.name}
                  </div>
        ...
        <div className="w-full lg:w-[310px] shrink-0 bg-white rounded-2xl p-5 shadow-[0_4px_16px_rgba(0,0,0,0.09)] h-fit lg:sticky lg:top-6">
          ...
          <h2 className="m-0 mb-4 text-[17px]">
            🛒 Your Order
          </h2>
            ...
            cart.map((item, i) => (
              ...
              >
                ...
                <button
                  ...
                  className="w-6 h-6 rounded-full border-none bg-gray-100 hover:bg-gray-200 cursor-pointer flex items-center justify-center"
                >
                  −
                </button>
                ...
                {trains.map(train => {
                  ...
                  return (
                    <div
                      ...
                      className={`rounded-lg p-2 cursor-${
                        isCancelled
                          ? "not-allowed"
                          : "pointer"
                      } transition-all`}
